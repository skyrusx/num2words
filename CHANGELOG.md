...
### Added
- Полная актуализация всех локалей до единого набора возможностей: числа, дроби, даты, время, дата-время и валюты.
- Единый каталог из 32 валют для каждой локали:
  `BDT`, `BGN`, `BRL`, `BYN`, `CNY`, `CZK`, `DKK`, `EUR`, `GBP`, `HUF`, `IDR`, `ILS`, `INR`, `IRR`, `JPY`, `KES`, `KRW`, `KZT`, `MYR`, `NOK`, `PKR`, `PLN`, `RON`, `RSD`, `RUB`, `SAR`, `SEK`, `THB`, `TRY`, `UAH`, `USD`, `VND`.
- Отдельные specs для локалей с проверками чисел, дробей, валют, дат и времени.
- Проверка `currency_catalog_spec.rb`, которая гарантирует одинаковый и уникальный набор валют во всех актуализированных локалях.

### Changed
- Языковые правила и грамматические особенности перенесены в модули `Num2words::Locales::XX`.
- `Converter` оставлен для общего поведения и делегирует языковые различия локалям через locale hooks.
- README актуализирован под текущий набор возможностей и локалей.
- Описание gemspec обновлено: поддержка больше не ограничивается первичной русской локалью.

### Fixed
- Устранены legacy-локали без `GRAMMAR`.
- Все локали теперь проходят smoke coverage без pending.
- Исправлены и унифицированы данные валют, дат, времени и дробей в локалях.
